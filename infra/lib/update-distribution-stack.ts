...
export class UpdateDistributionStack extends Stack {
  public constructor(scope: Construct, id: string, props: UpdateDistributionStackProps) {
    ...
    if (this.region !== "us-east-1") {
      throw new Error("Deploy this stack in us-east-1 so CloudFront can use its ACM certificate");
    }
    ...
    const artifactOrigin = origins.S3BucketOrigin.withOriginAccessControl(artifactBucket);
    ...
    const mutableFeedBehavior: cloudfront.BehaviorOptions = {
      ...defaultBehavior,
    ...
    const distribution = new cloudfront.Distribution(this, "Distribution", {
      ...
      additionalBehaviors: {
        "appcast.xml": mutableFeedBehavior,
        "staging/appcast.xml": mutableFeedBehavior,
        "releases/latest/*": mutableFeedBehavior,
      },
    });
    ...
    const publisherRole = new iam.Role(this, "GitHubPublisherRole", {
      ...
      description: "Publishes signed SpaceRabbit updates from the protected GitHub environment",
    ...
    publisherRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        conditions: {
          StringLike: {
            "s3:prefix": ["appcast.xml", "releases/*", "staging/*"],
          },
        },
        resources: [artifactBucket.bucketArn],
      }),
    ...
    publisherRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          ...
          artifactBucket.arnForObjects("staging/*"),
        ],
      }),
    );
    publisherRole.addToPolicy(
