...
auto already_running_error() -> error {
  return make_error(error_code::already_running, "SpaceHound daemon runtime is already running.");
}

auto not_running_error() -> error {
  return make_error(error_code::not_running, "SpaceHound daemon runtime is not running.");
}
...
struct deferred_action final {
  ...
  // The runtime's marked source, retained: the action may run after the
  // runtime that owns it has been stopped.
  cg::event_source source;
};
...
void run_deferred_action(void *raw_action) {
  ...
  (void)control::detail::execute_request(action->request, action->source.view());
}
...
void execute_hotkey(const detail::compiled_hotkey &hotkey, cg::event_source_view source) {
  dispatch::to_main(
      run_deferred_action,
      new deferred_action{
          .request = hotkey.request,
          .source = cg::event_source::retain(source),
      });
}
      ...
      [](CGEventTapProxy /*proxy*/, CGEventType type, CGEventRef event_ref, void *user_info)
          -> CGEventRef {
            ...
            for (const auto &hotkey : context->config.active_hotkeys) {
              ...
              execute_hotkey(hotkey, context->synthetic_source.view());
              return nullptr;
            }
...
auto runtime::start(const options &options) -> std::expected<void, error> {
  ...
  if (!initialize_runtime(
    ...
    return std::unexpected(
        runtime_error("Failed to initialize the SpaceHound daemon runtime."));
  }
