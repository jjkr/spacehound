...
typedef struct sh_tap_context {
  CFMachPortRef tap_ref;
  CGEventSourceRef synthetic_source_ref;
} sh_tap_context;

typedef struct sh_space_bounds {
...
static const int64_t SH_SYNTHETIC_EVENT_MARKER = 0x5348544150494E47LL;
...
static const char *sh_direction_name(enum sh_direction direction) {
  switch (direction) {
...
static int sh_is_horizontal_direction(enum sh_direction direction) {
  return direction == SH_DIRECTION_LEFT || direction == SH_DIRECTION_RIGHT;
}

static int sh_get_cf_number_int64(CFDictionaryRef dict_ref, CFStringRef key_ref, int64_t *out_value) {
...
static int sh_get_active_display_space_bounds(sh_space_bounds *out_bounds) {
  ...
  for (display_index = 0; display_index < display_count; ++display_index) {
    ...
    if (!sh_get_cf_number_int64(current_space_ref, managed_space_id_key, &current_space_id)) {
      break;
    }
    ...
    for (space_index = 0; space_index < space_count; ++space_index) {
      ...
      if (!sh_get_cf_number_int64(space_dict_ref, managed_space_id_key, &space_id)) {
        continue;
      }
...
static int sh_decode_direction(CGEventRef event_ref, enum sh_direction *out_direction) {
  const int64_t gesture_type =
      CGEventGetIntegerValueField(event_ref, SH_GESTURE_TYPE_FIELD);
  const double delta = CGEventGetDoubleValueField(event_ref, SH_GESTURE_DELTA_FIELD);
  ...
  switch (gesture_type) {
    case 1:
      *out_direction = (delta > 0.0) ? SH_DIRECTION_RIGHT : SH_DIRECTION_LEFT;
      return 1;
    case 2:
      *out_direction = (delta > 0.0) ? SH_DIRECTION_UP : SH_DIRECTION_DOWN;
      return 1;
    default:
      return 0;
  }
...
static int sh_decode_phase(CGEventRef event_ref, enum sh_gesture_phase *out_phase) {
  const int64_t phase = CGEventGetIntegerValueField(event_ref, SH_GESTURE_PHASE_FIELD);
  ...
  switch (phase) {
    case SH_GESTURE_PHASE_BEGIN:
      *out_phase = SH_GESTURE_PHASE_BEGIN;
      return 1;
    case SH_GESTURE_PHASE_UPDATE:
      *out_phase = SH_GESTURE_PHASE_UPDATE;
      return 1;
    case SH_GESTURE_PHASE_END:
      *out_phase = SH_GESTURE_PHASE_END;
      return 1;
    default:
      return 0;
  }
...
static int sh_post_fast_swipe(sh_tap_context *context, CGEventTapProxy proxy, enum sh_direction direction) {
  return sh_post_gesture_phase(context, proxy, SH_GESTURE_PHASE_BEGIN, direction) &&
         sh_post_gesture_phase(context, proxy, SH_GESTURE_PHASE_UPDATE, direction) &&
         sh_post_gesture_phase(context, proxy, SH_GESTURE_PHASE_END, direction);
}

static CGEventRef sh_event_tap_callback(
    CGEventTapProxy proxy,
    CGEventType type,
    CGEventRef event_ref,
    void *user_info) {
  sh_tap_context *context = (sh_tap_context *)user_info;
  enum sh_gesture_phase phase;
  enum sh_direction direction;
  ...
  if (marker == SH_SYNTHETIC_EVENT_MARKER) {
    return event_ref;
  }

  if (!sh_decode_phase(event_ref, &phase) || !sh_decode_direction(event_ref, &direction)) {
    return event_ref;
  }

  if (phase == SH_GESTURE_PHASE_BEGIN) {
    if (sh_is_horizontal_direction(direction)) {
      sh_space_bounds bounds;

      if (sh_get_active_display_space_bounds(&bounds)) {
        if (direction == SH_DIRECTION_LEFT && bounds.current_index == 0) {
          fprintf(stdout, "Ignoring left begin gesture at first space.\n");
          fflush(stdout);
          return NULL;
        }

        if (direction == SH_DIRECTION_RIGHT &&
    ...
    fprintf(stdout, "Intercepted begin gesture: %s\n", sh_direction_name(direction));
    fflush(stdout);

    if (!sh_post_fast_swipe(context, proxy, direction)) {
...
int main(int argc, char **argv) {
  ...
  CGEventSourceSetUserData(context.synthetic_source_ref, SH_SYNTHETIC_EVENT_MARKER);
