...
auto make_temp_dir() -> fs::path {
  ...
  const auto path = fs::temp_directory_path() /
                    ("spacehound-settings-tests-" + std::to_string(++counter));
  fs::remove_all(path);
...
TEST(settings_tests, default_path_prefers_lowercase_xdg_directory) {
  ...
  EXPECT_EQ(*path, xdg_dir / "spacehound" / "settings.json");
}
...
TEST(settings_tests, default_path_falls_back_to_macos_application_support) {
  ...
  EXPECT_EQ(
      *path,
      home_dir / "Library" / "Application Support" / "SpaceHound" / "settings.json");
}
