...
| An old beta is still a GitHub pre-release after a later version was promoted | Promotion already dropped it from the feed, so the pre-release flag is cosmetic. Flipping it is harmless: `gh release edit vX.Y.Z --prerelease=false --latest=false` triggers a **Promote** run that fails at the feed check before touching anything, and Sparkle never offers a version older than the installed one. Pass `--latest=false` so the Latest badge stays on the newest production release. |
