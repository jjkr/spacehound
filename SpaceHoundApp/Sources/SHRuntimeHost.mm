...
@property(nonatomic, copy, nullable) NSString *startError;
...
- (void)updateMenuBarTitle:(NSString *)menuBarTitle startError:(nullable NSString *)startError;
...
- (void)start {
  ...
  [self updateMenuBarTitle:@"" startError:nil];
  ...
  if (![self startRuntime:&runtimeError]) {
    ...
    [self updateMenuBarTitle:@"" startError:message];
    return;
  }
...
- (void)handleWorkspaceStateChangeWithCurrentSpace:(NSUInteger)currentSpace
  ...
  [self updateMenuBarTitle:[NSString stringWithFormat:@"%lu", (unsigned long)currentSpace] startError:nil];
}
...
- (void)updateMenuBarTitle:(NSString *)menuBarTitle startError:(nullable NSString *)startError {
  ...
  _startError = [startError copy];

  if (self.stateChangeHandler != nil) {
    self.stateChangeHandler(_menuBarTitle, _startError);
  }
}
