...
auto make_error(NSInteger code, NSString *description) -> NSError * {
  return [NSError errorWithDomain:SHSettingsStoreErrorDomain
                             code:code
...
@implementation SHHotkeyItem
@end

@implementation SHSettingsDocument
@end

@implementation SHSettingsStore
...
+ (NSURL *)settingsFileURL:(NSError **)error {
  ...
  NSURL *spaceHoundDirectory = [applicationSupportDirectory URLByAppendingPathComponent:@"SpaceHound"
                                                                              isDirectory:YES];
  if (![fileManager createDirectoryAtURL:spaceHoundDirectory
  ...
  NSURL *settingsURL = [spaceHoundDirectory URLByAppendingPathComponent:@"settings.json"];
  if (![fileManager fileExistsAtPath:settingsURL.path]) {
    ...
    os_log_info(SHLogSettings(), "Created default settings file");
  }

  return settingsURL;
}

+ (SHSettingsDocument *)loadDocument:(NSError **)error {
  NSURL *settingsURL = [self settingsFileURL:error];
  if (settingsURL == nil) {
    return nil;
  }

  const auto loaded = spacehound::settings::load(settings_file_path(settingsURL));
  if (!loaded.has_value()) {
    if (error != NULL) {
  ...
  const spacehound::settings::document &parsed = *loaded;
