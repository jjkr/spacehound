...
namespace daemon = spacerabbit::daemon;
namespace detail = spacerabbit::daemon::detail;
...
auto make_temp_dir() -> fs::path {
  ...
  const auto path =
      fs::temp_directory_path() / ("spacerabbit-daemon-tests-" + std::to_string(++counter));
  fs::remove_all(path);
...
TEST(daemon_tests, resolve_settings_path_prefers_override) {
  const fs::path expected = "/tmp/spacerabbitd-settings.json";
