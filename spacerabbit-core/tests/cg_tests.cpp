...
TEST(cg_tests, mouse_and_keyboard_event_creation_helpers_work) {
  ...
  if (std::getenv("SPACERABBIT_ENABLE_LIVE_QUARTZ_TESTS") == nullptr) {
    GTEST_SKIP() << "Set SPACERABBIT_ENABLE_LIVE_QUARTZ_TESTS=1 to exercise live Quartz event creation.";
  }
