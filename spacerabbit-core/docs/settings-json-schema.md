# SpaceRabbit Settings JSON Schema

## Purpose

This document defines the canonical shared `settings.json` contract for SpaceRabbit.

It should be treated as the target schema for:

- `~/work/spacerabbit` as the GUI/editor and primary writer
- `~/work/spacerabbit-core` as the daemon/runtime reader
...
The current AppKit menu bar app writes this file through `SRSettingsStore`, and
the in-process C++ runtime reads it through `spacerabbit::settings`. Both sides
...
```json
{
  ...
  "$id": "https://spacerabbit.app/schemas/settings-1.0.json",
  "title": "SpaceRabbit Settings",
  "type": "object",
  "required": [
    "version",
    "workspaceWrap",
