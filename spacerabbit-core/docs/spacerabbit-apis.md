# SpaceRabbit macOS API Inventory

This document inventories the macOS APIs used by SpaceRabbit, based on the current native app and C++ runtime code in:

- `SpaceRabbitApp/Sources/`
- `spacerabbit-core/include/`
- `spacerabbit-core/lib/`
...
SpaceRabbit depends on a small set of macOS frameworks to do four things:
...
- `AXUIElementCreateApplication(pid)`
  ...
  - Used when SpaceRabbit focuses a specific window in another app.
...
- `AXUIElement::application_with_bundle("com.apple.dock")`
  - Gets the Dock's AX root so SpaceRabbit can inspect Mission Control and Expose.
...
- `CGEventGetIntegerValueField` / `CGEventSetIntegerValueField`
  ...
  - Public for normal fields, but SpaceRabbit also uses it with private field IDs for gestures.
...
- `addObserverForName:object:queue:usingBlock:` with `NSWorkspaceActiveSpaceDidChangeNotification`
  ...
  - SpaceRabbit uses this to keep the tray title synced with the current Space number.
...
SpaceRabbit wraps these in `dispatch_to_main(...)` so window-focus restoration can hop back to the main thread. That matters because AppKit-triggering work, especially the AX raise/focus sequence, is safest on the main thread.
...
- SpaceRabbit treats that as the active display for display-cycling and tray state.
...
In practice, this undocumented event shape is what lets SpaceRabbit replace normal system swipes with faster or more controlled synthetic Space/Mission Control gestures.

## Runtime Behavior Tied To These APIs

### Switching Spaces

SpaceRabbit does not call a public "switch to Space N" API, because macOS does not expose one. Instead it:
...
These exist in `~/work/spacerabbit`, but I did not find them used from the main startup/hotkey/tray flow:
...
SpaceRabbit is fundamentally a macOS-native control app built around:
